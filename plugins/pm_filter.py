    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("𝑷𝒂𝒈𝒆𝒔 📖", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("𝑷𝒂𝒈𝒆𝒔 📖", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("𝑷𝒓𝒆𝒗𝒊𝒐𝒖𝒔 ⏪", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    if files:
        ...
        if offset != "":
            ...
            try:
                if settings['max_btn']:
                    btn.append(
                        [InlineKeyboardButton("𝑷𝒂𝒈𝒆𝒔 📖", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                    )

                else:
                    btn.append(
                        [InlineKeyboardButton("𝑷𝒂𝒈𝒆𝒔 📖", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                    )
            except KeyError:
                await save_group_settings(query.message.chat.id, 'max_btn', True)
                btn.append(
            ...
            btn.append(
                [InlineKeyboardButton(text="𝑵𝒐 𝒎𝒐𝒓𝒆 𝒑𝒂𝒈𝒆𝒔 𝒂𝒗𝒂𝒊𝒍𝒂𝒃𝒍𝒆 ⚕️",callback_data="pages")]
            )
        try:
            await query.edit_message_reply_markup(
                reply_markup=InlineKeyboardMarkup(btn)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Join the channel mahn 😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        if file_id == "send_all":
        ...
        buttons = [[
            InlineKeyboardButton('𝑮𝒓𝒐𝒖𝒑 📣', url=GRP_LNK),
            InlineKeyboardButton('𝑺𝒐𝒖𝒓𝒄𝒆 💡', callback_data='source')
        ],[
            InlineKeyboardButton('𝑯𝒐𝒎𝒆 🏠', callback_data='start'),
        ...
        buttons = [[
            InlineKeyboardButton('⬅️ 𝑩𝒂𝒄𝒌', callback_data='manuelfilter')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton('⬅️ 𝑩𝒂𝒄𝒌', callback_data='help'),
            InlineKeyboardButton('𝑨𝒅𝒎𝒊𝒏 👷‍♂️', callback_data='admin')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton('⬅️ 𝑩𝒂𝒄𝒌', callback_data='extra')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("𝑪𝒐𝒏𝒕𝒂𝒄𝒕 👷‍♂️", url="t.me/creatorbeatz")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>𝐻𝑦𝑦 😍 {message.from_user.mention},👋🏻 /n 📬 𝑇𝑖𝑡𝑙𝑒 {search} /n 🔰 𝑇ℎ𝑥 𝐹𝑜𝑟 𝑅𝑒𝑞𝑢𝑒𝑠𝑡 🎯</b>"
            ...
            try:
                if settings['auto_delete']:
                    ...
                    await message.reply_text(
                        text=f"👋 ഹൈയ് {message.from_user.mention}/nനിങ്ങളുടെ അഭ്യർത്ഥന ഇല്ലാതാക്കി👍🏻/n(പകർപ്പവകാശ പ്രശ്നം ഒഴിവാക്കുന്നതിനാൽ)/nആ ഫയലുകൾക്കായി വീണ്ടും അഭ്യർത്ഥിക്കുക ❤️/n👋 𝐻𝑦𝑦 {message.from_user.mention}/n𝑌𝑜𝑢𝑟 𝑅𝑒𝑞𝑢𝑒𝑠𝑡 𝐻𝑎𝑠 𝐵𝑒𝑒𝑛 𝐷𝑒𝑙𝑒𝑡𝑒𝑑👍🏻/n(𝐷𝑢𝑒 𝑇𝑜 𝐴𝑣𝑜𝑖𝑑 𝐶𝑜𝑝𝑦𝑟𝑖𝑔ℎ𝑡𝑠 𝐼𝑠𝑠𝑢𝑒😌)/n𝑅𝑒𝑞𝑢𝑒𝑠𝑡 𝐴𝑔𝑎𝑖𝑛 𝐹𝑜𝑟 𝑇ℎ𝑎𝑡 𝐹𝑖𝑙𝑒𝑠 ❤️")                         
            except KeyError:
                await save_group_settings(message.chat.id, 'auto_delete', True)
                await asyncio.sleep(600)
                await fek.delete()
